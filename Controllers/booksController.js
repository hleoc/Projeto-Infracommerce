...
books.get('/search', auth, async (req, res) => {
  try {
    const { title, author, category, registrationDate, dateOfChange } = req.query;
    const bookDetails = await service.getAllDetails(title, author, category, registrationDate, dateOfChange);
